...
def(var(Nom,T),G,[(Nom,T)|G]).
def(proc(Nom,Ar,B),G,[(Nom,typefun(Ts,void))|G]):-getTypesArgs(Ar,Ts),append(Ar,G,ArG),block(B,ArG,void).%pareil que pour deffun
def(procRec(Nom,Ar,B),G,[(Nom,typefun(Ts,void))|G]):-getTypesArgs(Ar,Ts),append(Ar,G,ArG),block(B,[(Nom,typefun(Ts,void))|ArG],void).
...
stat(call(Nom,Es),G,void):-typesliste(G,Es,Ts),assoc(Nom,G,typefun(Ts,void)).
